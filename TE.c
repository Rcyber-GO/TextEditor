...
// KARAKTER BIASA
            ...
            // AUTO-WRAP (Pindah baris otomatis jika kursor di ujung kanan)
            ...
            // CEK OVERFLOW (Pencegah Error Layar Berantakan)
            // Jika teks di baris ini sudah penuh maksimal, blokir ketikan tambahan di tengah kalimat
            if (len >= MAX_KOLOM - 2) {
                continue; 
            }
...
void bukaFile(TextEditor *ed) {
    ...
    inisialisasiEditor(ed); // Reset editor sebelum load file
    ...
    if (ed->jumlah_baris == 0) ed->jumlah_baris = 1; // Minimal 1 baris kosong
